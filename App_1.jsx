...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        const unsubData = subscribeUserData(firebaseUser.uid, (d) => {
          setUserData(d);
        });
        return () => unsubData();
      } else {
  ...
  // updateData: uses ref so always has current uid
  const updateData = useCallback(async (newData) => {
    setUserData(newData);
    const currentUser = auth.currentUser;
    if (currentUser) {
      try {
        await saveUserData(currentUser.uid, newData);
      } catch(e) {
  ...
  if (dataLoading || !userData) {
    return <LoadingScreen text="Cargando tus datos…" />;
  }

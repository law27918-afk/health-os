...
const USERS_KEY = "hos_users";
const SESSION_KEY = "hos_session";
const DATA_KEY = (uid) => `hos_data_${uid}`;

function getUsers() { return S.get(USERS_KEY, {}); }
function saveUsers(u) { S.set(USERS_KEY, u); }
function getSession() { return S.get(SESSION_KEY, null); }
function saveSession(uid) { S.set(SESSION_KEY, uid); }
function clearSession() { S.del(SESSION_KEY); }
function getUserData(uid) {
  return S.get(DATA_KEY(uid), {
    metrics: defaultMetrics(),
    logs: {},
    profile: {},
  });
}
function saveUserData(uid, data) { S.set(DATA_KEY(uid), data); }
...
export default function App() {
  ...
  const [userData, setUserData] = useState(() => uid ? getUserData(uid) : null);
  ...
  const updateData = useCallback((newData) => {
    setUserData(newData);
    if (uid) saveUserData(uid, newData);
  }, [uid]);

  if (!uid) {
    return <AuthScreen mode={authMode} setMode={setAuthMode} onLogin={login} />;
  }
...
// ─── Shell / Navigation ───────────────────────────────────────────────────────
function Shell({ view, setView, userData, updateData, uid, onLogout }) {
  const navItems = [
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#f8fafc", fontFamily: "system-ui, -apple-system, sans-serif" }}>
        ...
        <nav style={{ flex: 1, padding: "16px 12px", display: "flex", flexDirection: "column", gap: 2 }}>
          {navItems.map(n => (
            <button key={n.id} onClick={() => setView(n.id)} style={{
              ...
              color: view === n.id ? "#0f172a" : "#64748b",
              fontWeight: view === n.id ? 600 : 400, fontSize: 14,
      ...
      <main style={{ marginLeft: 220, flex: 1, minHeight: "100vh" }} className="main-content">
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px 100px" }}>
          ...
          {view === "profile" && <ProfileView uid={uid} userData={userData} updateData={updateData} onLogout={onLogout} />}
        </div>
      </main>
      ...
      <nav style={{
        ...
        borderTop: "1px solid #f1f5f9", display: "flex", padding: "8px 0 12px",
        zIndex: 20,
        ...
        {navItems.map(n => (
          <button key={n.id} onClick={() => setView(n.id)} style={{
            ...
            color: view === n.id ? "#6366f1" : "#94a3b8", fontSize: 10, fontWeight: view === n.id ? 600 : 400,
          }}>
      ...
      <style>{`
        ...
        @media(max-width:768px){.sidebar-desktop{display:none!important}.main-content{margin-left:0!important;padding:16px 16px 90px}}
...
function AuthScreen({ mode, setMode, onLogin }) {
  ...
  const handleSubmit = () => {
    ...
    if (resetMode) {
      if (!users[email]) { setError("No existe una cuenta con ese correo."); return; }
      setSuccess("Se ha enviado un enlace de recuperación (simulado). Usa tu contraseña actual.");
      return;
    }

    if (mode === "login") {
      if (!users[email]) { setError("Correo o contraseña incorrectos."); return; }
      if (users[email].password !== password) { setError("Correo o contraseña incorrectos."); return; }
      onLogin(email);
    } else {
      if (!name.trim()) { setError("Ingresa tu nombre."); return; }
      if (!email.includes("@")) { setError("Correo inválido."); return; }
      if (password.length < 6) { setError("La contraseña debe tener al menos 6 caracteres."); return; }
      if (users[email]) { setError("Ya existe una cuenta con ese correo."); return; }
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "system-ui, sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #f1f5f9", padding: "32px 28px", boxShadow: "0 4px 24px rgba(0,0,0,0.04)" }}>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 6 }}>Correo electrónico</label>
            <input value={email} onChange={e => setEmail(e.target.value)} placeholder="tu@correo.com" type="email" style={inputStyle} />
          </div>
          {!resetMode && (
          ...
          <button onClick={handleSubmit} style={{ ...btnPrimary, width: "100%", padding: "12px", fontSize: 15 }}>
            {resetMode ? "Enviar enlace" : mode === "login" ? "Entrar" : "Crear cuenta"}
          </button>
          ...
          {resetMode ? (
            <button onClick={() => setResetMode(false)} style={{ background: "none", border: "none", color: "#6366f1", cursor: "pointer", fontSize: 13, display: "block", margin: "12px auto 0" }}>
              ← Volver
            </button>
          ) : (
...
function Dashboard({ userData, updateData, setView }) {
  ...
  return (
    <div>
      ...
      {pending.length > 0 && (
        <section style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {pending.map(m => (
              <MetricCard key={m.id} metric={m} log={logs[m.id]} updateData={updateData} userData={userData} todayKey={todayKey} />
            ))}
          </div>
        </section>
      )}
      ...
      {metrics.length === 0 && (
        <EmptyState icon="ti-adjustments" title="Sin métricas aún" body="Configura tus métricas de salud para empezar." />
      )}
    </div>
  );
}
...
function QuickActionBtn({ icon, label, color, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      background: "#fff", border: "1px solid #f1f5f9", borderRadius: 12, cursor: "pointer",
      fontSize: 14, fontWeight: 600, color: "#0f172a", transition: "all 0.15s",
    }}>
...
function MetricCard({ metric, log, updateData, userData, todayKey }) {
  ...
  function patchLog(newLog) {
    const data = { ...userData };
    data.logs = { ...data.logs };
    data.logs[todayKey] = { ...data.logs[todayKey] };
    data.logs[todayKey][metric.id] = { ...data.logs[todayKey][metric.id], ...newLog };
    updateData(data);
  }

  return (
    <div style={{
      background: "#fff", borderRadius: 14, border: `1px solid ${done ? "#d1fae5" : "#f1f5f9"}`,
      ...
      {/* Progress bar for quantity/counter/time */}
      {(metric.type === "quantity" || metric.type === "counter" || metric.type === "time") && metric.goal && (
        <div style={{ height: 3, background: "#f1f5f9", margin: "0 16px 0" }}>
          <div style={{
            height: "100%", borderRadius: 2,
            background: col.accent,
            width: `${Math.min(100, ((log?.amount || log?.count || log?.minutes || 0) / metric.goal) * 100)}%`,
...
function MetricInput({ metric, log, patchLog }) {
  ...
  if (metric.type === "check") {
    return (
      <button onClick={() => patchLog({ done: true, time: new Date().toISOString() })} style={{ ...btnPrimary, width: "100%" }}>
  ...
  if (metric.type === "counter") {
    ...
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <button onClick={() => patchLog({ count: Math.max(0, cur - 1) })} style={{ ...btnSecondary, width: 40, height: 40, borderRadius: "50%", padding: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>−</button>
        ...
        <button onClick={() => patchLog({ count: cur + 1 })} style={{ ...btnPrimary, width: 40, height: 40, borderRadius: "50%", padding: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>+</button>
      </div>
    );
  }
    ...
    return (
      <div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginBottom: 12 }}>
          {quick.map(q => (
            <button key={q} onClick={() => patchLog({ amount: cur + q })} style={{ ...btnSecondary, fontSize: 13, padding: "8px 4px" }}>
              +{q < 1000 ? q : q / 1000 + "L"}{metric.unit === "ml" && q < 1000 ? "ml" : ""}
            </button>
          ))}
        </div>
  ...
  if (metric.type === "time") {
    const [h, setH] = useState(local.h || "");
    const [m_, setM] = useState(local.m || "");
    return (
      <div>
        ...
        }} style={{ ...btnPrimary, width: "100%" }}>Guardar</button>
  ...
  if (metric.type === "scale") {
    ...
    return (
      <div>
        ...
        <input type="range" min={metric.min || 1} max={metric.max || 10} value={cur || metric.min || 1}
          onChange={e => patchLog({ value: Number(e.target.value) })} style={{ width: "100%" }} />
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
          {Array.from({ length: (metric.max || 10) - (metric.min || 1) + 1 }, (_, i) => i + (metric.min || 1)).map(v => (
            <button key={v} onClick={() => patchLog({ value: v })} style={{
              width: 36, height: 36, borderRadius: 8, border: `1px solid ${cur === v ? "#6366f1" : "#e2e8f0"}`,
              background: cur === v ? "#6366f1" : "#fff", color: cur === v ? "#fff" : "#0f172a",
...
// ─── PENDING VIEW ─────────────────────────────────────────────────────────────
function PendingView({ userData, updateData }) {
  const todayKey = today();
  const logs = userData.logs?.[todayKey] || {};
  const metrics = (userData.metrics || []).filter(m => !m.archived);
  const pending = metrics.filter(m => !isMetricDone(m, logs[m.id]));
  ...
  return (
    <div>
      ...
      <p style={{ color: "#64748b", marginBottom: 24 }}>
        {pending.length === 0 ? "¡Todo completado hoy! 🎉" : `${pending.length} pendiente${pending.length > 1 ? "s" : ""}`}
      </p>
...
function LogView({ userData, updateData }) {
  ...
  function patchLog(metricId, newLog) {
    ...
    data.logs = { ...data.logs };
    data.logs[todayKey] = { ...data.logs[todayKey] };
    data.logs[todayKey][metricId] = { ...data.logs[todayKey][metricId], ...newLog };
    updateData(data);
  }
  ...
  if (selectedMetric) {
    ...
    return (
      <div>
        <button onClick={() => setSelectedMetric(null)} style={{ ...btnSecondary, marginBottom: 20, display: "flex", alignItems: "center", gap: 6 }}>
        ...
        {/* Exercise extra fields */}
        {m.id === "ejercicio" && (
          <ExerciseLogger metric={m} log={logs[m.id]} patchLog={p => patchLog(m.id, p)} />
        )}
      </div>
    );
  }
        ...
        {metrics.map(m => {
          ...
          return (
            <button key={m.id} onClick={() => setSelectedMetric(m.id)} style={{
              ...
              padding: 16, background: "#fff", border: `1px solid ${done ? "#d1fae5" : "#f1f5f9"}`,
...
function ExerciseLogger({ metric, log, patchLog }) {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #f1f5f9", padding: 24, marginTop: 16 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {types.map(t => (
            <button key={t} onClick={() => setType(t)} style={{
              padding: "6px 14px", borderRadius: 8, border: `1px solid ${type === t ? "#6366f1" : "#e2e8f0"}`,
              background: type === t ? "#6366f1" : "#fff", color: type === t ? "#fff" : "#374151",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
        <div>
          <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 6 }}>Duración (min)</label>
          <input type="number" value={duration} onChange={e => setDuration(e.target.value)} placeholder="30" style={inputStyle} />
        </div>
        <div>
          <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 6 }}>Intensidad</label>
      ...
      <button onClick={() => patchLog({ exerciseType: type, duration: Number(duration), intensity, notes, done: true })} style={{ ...btnPrimary, width: "100%" }}>
        Guardar ejercicio
      </button>
    </div>
  );
...
function TrendsView({ userData }) {
  ...
  const scores = days.map(d => ({
    date: d,
    score: calcScore(metrics, userData.logs?.[d] || {}),
  }));
  ...
  const bestDay = scores.find(d => d.score === maxScore);
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
        {[7, 30, 90].map(n => (
          <button key={n} onClick={() => setRange(n)} style={{
            padding: "6px 16px", borderRadius: 8, border: `1px solid ${range === n ? "#6366f1" : "#e2e8f0"}`,
            background: range === n ? "#6366f1" : "#fff", color: range === n ? "#fff" : "#374151",
            cursor: "pointer", fontSize: 14, fontWeight: range === n ? 600 : 400,
          }}>{n} días</button>
        ))}
      </div>
...
function MiniBarChart({ data, days }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height: h, paddingBottom: 24, position: "relative" }}>
      {/* Guideline */}
      {[25, 50, 75, 100].map(g => (
        <div key={g} style={{ position: "absolute", left: 0, right: 0, bottom: 24 + (g / 100) * (h - 24) - 1, borderTop: "1px dashed #f1f5f9", display: "flex", alignItems: "center" }}>
          <span style={{ fontSize: 10, color: "#cbd5e1", paddingRight: 4 }}>{g}</span>
        </div>
      ))}
      {data.map((d, i) => {
        ...
        return (
          <div key={d.date} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
            <div title={`${d.date}: ${d.score}`} style={{
              ...
              background: isToday ? "#6366f1" : (d.score > 0 ? color : "#f1f5f9"),
              transition: "height 0.4s",
            }} />
            {days <= 14 && (
              <div style={{ fontSize: 9, color: isToday ? "#6366f1" : "#cbd5e1", fontWeight: isToday ? 700 : 400, whiteSpace: "nowrap" }}>
                {new Date(d.date + "T12:00").toLocaleDateString("es-ES", { weekday: "narrow" })}
              </div>
            )}
          </div>
        );
...
function MetricsManager({ userData, updateData }) {
  ...
  function blankMetric() {
    return { name: "", icon: "ti-circle", type: "check", goal: "", unit: "", weight: 10, color: "blue", min: 1, max: 10 };
  }
  ...
  function deleteMetric(id) {
    ...
    updateData({ ...userData, metrics: metrics.filter(m => m.id !== id) });
  }

  function toggleArchive(m) {
    updateData({ ...userData, metrics: metrics.map(x => x.id === m.id ? { ...x, archived: !x.archived } : x) });
  }

  function duplicateMetric(m) {
    const copy = { ...m, id: `m_${Date.now()}`, name: m.name + " (copia)", archived: false };
    updateData({ ...userData, metrics: [...metrics, copy] });
  }
  ...
  if (adding) {
    return (
      <div>
        <button onClick={() => { setAdding(false); setEditing(null); setForm(blankMetric()); }} style={{ ...btnSecondary, marginBottom: 20, display: "flex", alignItems: "center", gap: 6 }}>
          <i className="ti ti-arrow-left" style={{ fontSize: 16 }} /> Volver
        </button>
        <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #f1f5f9", padding: 24 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: "#0f172a", margin: "0 0 20px" }}>
            {editing ? "Editar métrica" : "Nueva métrica"}
          </h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            ...
            {form.type === "scale" && (
              <>
                <div>
                  ...
                  <input type="number" value={form.min} onChange={e => setForm(f => ({ ...f, min: Number(e.target.value) }))} style={inputStyle} />
                </div>
                <div>
                  <label style={labelStyle}>Máximo</label>
                  <input type="number" value={form.max} onChange={e => setForm(f => ({ ...f, max: Number(e.target.value) }))} style={inputStyle} />
                </div>
              </>
            )}
            <div>
            ...
            <div>
              ...
              <div style={{ marginTop: 6 }}>
                <i className={`ti ${form.icon}`} style={{ fontSize: 24, color: (COLOR_MAP[form.color] || COLOR_MAP.gray).accent }} />
              </div>
            </div>
          </div>

          <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
            <button onClick={saveMetric} style={{ ...btnPrimary, flex: 1 }}>
              {editing ? "Guardar cambios" : "Crear métrica"}
            </button>
            <button onClick={() => { setAdding(false); setEditing(null); setForm(blankMetric()); }} style={btnSecondary}>
              Cancelar
            </button>
          </div>
        </div>
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        ...
        <button onClick={() => setAdding(true)} style={btnPrimary}>
          <i className="ti ti-plus" style={{ marginRight: 6 }} /> Nueva métrica
        </button>
      </div>

      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 32 }}>
        {active.map(m => (
          <MetricRow key={m.id} metric={m} onEdit={startEdit} onDelete={deleteMetric} onArchive={toggleArchive} onDuplicate={duplicateMetric} />
        ))}
      ...
      {archived.length > 0 && (
        <>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {archived.map(m => (
              <MetricRow key={m.id} metric={m} onEdit={startEdit} onDelete={deleteMetric} onArchive={toggleArchive} onDuplicate={duplicateMetric} isArchived />
            ))}
          </div>
        </>
      )}
    ...
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: 12, color: "#94a3b8" }}>
          {typeLabel(metric.type)} · Peso: {metric.weight}pts
          {metric.goal ? ` · Meta: ${metric.goal}${metric.unit ? " " + metric.unit : ""}` : ""}
        </div>
      </div>
...
// ─── PROFILE VIEW ─────────────────────────────────────────────────────────────
function ProfileView({ uid, userData, updateData, onLogout }) {
  const users = getUsers();
  const user = users[uid] || {};
  ...
  function changePw() {
    setErr(""); setMsg("");
    const u = getUsers();
    if (u[uid].password !== currentPw) { setErr("Contraseña actual incorrecta."); return; }
    if (newPw.length < 6) { setErr("La nueva contraseña debe tener al menos 6 caracteres."); return; }
    u[uid] = { ...u[uid], password: newPw };
    saveUsers(u);
    ...
    <div>
      ...
      <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #f1f5f9", padding: 24, marginBottom: 20 }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input value={uid} disabled style={{ ...inputStyle, background: "#f8fafc", color: "#94a3b8" }} />
        </div>
      ...
      <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #f1f5f9", padding: 24, marginBottom: 20 }}>
        ...
        <button onClick={changePw} style={btnPrimary}>Actualizar contraseña</button>
      </div>

      <button onClick={onLogout} style={{ ...btnSecondary, color: "#ef4444", borderColor: "#fecaca", width: "100%" }}>
...
function metricStatus(m, log) {
  if (!log) {
    ...
    if (m.type === "scale") return `Sin registrar`;
    if (m.type === "number") return `Sin registrar`;
    return "Pendiente";
  }
...
const btnPrimary = {
  ...
  color: "#fff", fontWeight: 600, fontSize: 14, cursor: "pointer", display: "inline-flex", alignItems: "center",
};
const btnSecondary = {
  padding: "9px 18px", borderRadius: 9, border: "1px solid #e2e8f0",
  background: "#fff", color: "#374151", fontWeight: 500, fontSize: 14, cursor: "pointer", display: "inline-flex", alignItems: "center",
};
